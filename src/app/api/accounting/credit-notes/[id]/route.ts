/**
 * Credit Note Detail API - Get, Update, Delete (T084)
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!note.noteType.includes('credit')) {
      return NextResponse.json(
        { success: false, error: 'Not a credit note' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to get credit note' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to update credit note' },
      { status: 500 }
    );
  }
}
