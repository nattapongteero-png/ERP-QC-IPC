/**
 * VMI Scheduled Items Sync API
 *
 * POST /api/vmi-sync/scheduled/items - Triggered by cron for scheduled sync
 *
...
/**
 * POST /api/vmi-sync/scheduled/items
 *
 * Scheduled items sync (triggered by cron job)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[VMI Scheduled Sync] Items sync triggered by', isVercelCron ? 'Vercel Cron' : 'Custom Cron');

    // Run scheduled sync
    const result = await vmiSyncService.runScheduledItemsSync();
    ...
    console.error('[VMI Sync API] Error in scheduled items sync:', error);
