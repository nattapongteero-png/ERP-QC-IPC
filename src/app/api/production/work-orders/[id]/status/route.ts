  ...
  return withAuth(request, async (session) => {
    try {
      ...
      if (status === 'completed') {
        ...
        // Auto-create a Goods Receipt for the finished-goods output so it
        // appears on the GRN screen in quarantine, awaiting QC checklist before
        // the warehouse can release it to stock. The GRN's checklist step
        // creates the QC sample (so we no longer create a standalone sample
        // here — that would double-sample). Idempotent + best-effort.
        try {
          const { autoCreateGrnForSource } = await import('@/lib/services/goods-receipt.service');
          await autoCreateGrnForSource({ sourceType: 'wo', woId: workOrderId, userId: session.userId });
        } catch (err) {
          console.warn('WO-completed auto-GRN failed (non-fatal):', err);
        }
      }
