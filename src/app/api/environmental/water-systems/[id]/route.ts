/**
 * Water System — PUT (update) + DELETE (soft via isActive=false)
 */
...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  await executeDbOperation(async (db) => {
    const t = getTableRef('waterSystems');
    const patch: Record<string, unknown> = { updatedAt: getNow() };
    if (typeof body.name === 'string') patch.name = body.name;
    if (typeof body.systemType === 'string') patch.systemType = body.systemType;
    if (typeof body.description === 'string') patch.description = body.description;
    if (typeof body.isActive === 'boolean') patch.isActive = body.isActive;
    await db.update(t).set(patch).where(eq(t.id, wid));
  });
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  ...
  await executeDbOperation(async (db) => {
    const t = getTableRef('waterSystems');
    await db.update(t).set({ isActive: false, updatedAt: getNow() }).where(eq(t.id, wid));
  });
  return NextResponse.json({ success: true });
}
