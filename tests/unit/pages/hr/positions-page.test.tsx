/**
 * HR Positions Management Page Tests
 * Feature: 014-unit-cost
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { screen, waitFor } from '@testing-library/react';
import PositionsPage from '@/app/hr/positions/page';
import {
  renderWithProviders,
  setupFetchMock,
  clearFetchMock,
  createSingleResponse,
} from '../../../helpers/ui-test-utils';
import { MOCK_POSITIONS, MOCK_ORG_UNITS, HR_FETCH_HANDLERS } from '../../../helpers/fetch-mock-handlers';

// Mock next/navigation
vi.mock('next/navigation', () => ({
  useRouter: () => ({
    push: vi.fn(),
    back: vi.fn(),
  }),
}));

// Mock MainLayout
vi.mock('@/components/layout/main-layout', () => ({
  MainLayout: ({ children }: { children: React.ReactNode }) => <div data-testid="main-layout">{children}</div>,
}));

// Mock DevExtreme PieChart - the component uses named import PieChart
vi.mock('devextreme-react/pie-chart', () => {
  const MockPieChart = ({ children }: { children?: React.ReactNode }) => <div data-testid="pie-chart">{children}</div>;
  return {
    __esModule: true,
    default: MockPieChart,
    PieChart: MockPieChart,
    Series: () => null,
    Label: () => null,
    Legend: () => null,
    Tooltip: () => null,
    Connector: () => null,
    Size: () => null,
  };
});

// Mock DevExtreme Popup
vi.mock('devextreme-react/popup', () => ({
  __esModule: true,
  default: () => null,
  Popup: () => null,
  ToolbarItem: () => null,
}));

// Mock shared components
vi.mock('@/components/shared', () => ({
  ResponsivePageHeader: ({ title, subtitle }: { title: string; subtitle: string }) => (
    <div data-testid="responsive-header">
      <h1>{title}</h1>
      <p>{subtitle}</p>
    </div>
  ),
  StatCard: ({ label, value }: { label: string; value: string | number }) => (
    <div data-testid="stat-card">
      <span>{label}</span>
      <span>{value}</span>
    </div>
  ),
  OrgUnitPicker: () => <div data-testid="org-unit-picker" />,
}));

// Mock useToast
vi.mock('@/components/ui/toast', () => ({
  useToast: () => ({
    success: vi.fn(),
    error: vi.fn(),
  }),
}));

describe('PositionsPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    clearFetchMock();
  });

  describe('Page Rendering', () => {
    it('should render page with correct title', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(screen.getByText('ตำแหน่งงาน')).toBeInTheDocument();
      });
    });

    it('should render page subtitle', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(screen.getByText(/Position Management/)).toBeInTheDocument();
      });
    });

    it('should render positions page container', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        // The main page container
        expect(screen.getByTestId('hr-positions-page')).toBeInTheDocument();
      });
    });

    it('should render action buttons', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      // Page should render the header with actions
      await waitFor(() => {
        expect(screen.getByTestId('responsive-header')).toBeInTheDocument();
      });
    });

    it('should render main content area', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        // Check for the stats section
        expect(screen.getByTestId('hr-positions-stats')).toBeInTheDocument();
      });
    });
  });

  describe('Statistics Display', () => {
    it('should display stat cards', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(screen.getByText('ตำแหน่งทั้งหมด')).toBeInTheDocument();
        // "ใช้งาน" may appear multiple times (stat card + badges)
        expect(screen.getAllByText('ใช้งาน').length).toBeGreaterThan(0);
        expect(screen.getByText('GMP Critical')).toBeInTheDocument();
        expect(screen.getByText('หน่วยงาน')).toBeInTheDocument();
      });
    });
  });

  describe('Data Fetching', () => {
    it('should fetch positions on mount', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(fetch).toHaveBeenCalled();
      });

      // Verify the API was called with correct endpoint
      const calls = vi.mocked(fetch).mock.calls;
      const positionsCall = calls.find(([url]) =>
        typeof url === 'string' && url.includes('/api/hr/positions')
      );
      expect(positionsCall).toBeDefined();
    });

    it('should fetch org units on mount', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(fetch).toHaveBeenCalled();
      });

      // Verify the API was called with correct endpoint
      const calls = vi.mocked(fetch).mock.calls;
      const orgUnitsCall = calls.find(([url]) =>
        typeof url === 'string' && url.includes('/api/hr/org-units')
      );
      expect(orgUnitsCall).toBeDefined();
    });

    it('should handle empty positions data gracefully', async () => {
      setupFetchMock({
        '/api/hr/positions': { data: createSingleResponse([]) },
        '/api/hr/org-units': { data: createSingleResponse(MOCK_ORG_UNITS) },
      });

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(screen.getByText('ตำแหน่งงาน')).toBeInTheDocument();
      });
    });

    it('should handle API error gracefully', async () => {
      setupFetchMock({
        '/api/hr/positions': {
          data: { success: false, error: 'Server error' },
          ok: false,
          status: 500,
        },
        '/api/hr/org-units': { data: createSingleResponse(MOCK_ORG_UNITS) },
      });

      renderWithProviders(<PositionsPage />);

      // Page should still render even on API error
      await waitFor(() => {
        expect(screen.getByText('ตำแหน่งงาน')).toBeInTheDocument();
      });
    });

    it('should correctly parse direct array response structure (regression)', () => {
      // HR pages use direct array format, not paginated
      const mockResponse = createSingleResponse(MOCK_POSITIONS);

      // Correct: data returns the array directly
      expect(Array.isArray(mockResponse.data)).toBe(true);
      expect(mockResponse.data.length).toBe(MOCK_POSITIONS.length);
    });
  });

  describe('Filter Section', () => {
    it('should render filter toggle button via header', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      // Filter button is in the header section
      await waitFor(() => {
        expect(screen.getByTestId('responsive-header')).toBeInTheDocument();
      });
    });
  });

  describe('DataGrid Section', () => {
    it('should have data-testid for positions grid', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(screen.getByTestId('hr-positions-grid')).toBeInTheDocument();
      });
    });

    it('should have data-testid for positions stats', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(screen.getByTestId('hr-positions-stats')).toBeInTheDocument();
      });
    });
  });

  describe('Detail Panel', () => {
    it('should show detail panel placeholder when no position selected', async () => {
      setupFetchMock(HR_FETCH_HANDLERS);

      renderWithProviders(<PositionsPage />);

      await waitFor(() => {
        expect(screen.getByText('เลือกตำแหน่งเพื่อดูรายละเอียด')).toBeInTheDocument();
      });
    });
  });
});
